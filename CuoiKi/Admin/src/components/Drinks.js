...
const Drinks = () => {
  ...
  const handleUpdate = () => {
    axios
      .put(`http://localhost:5000/api/drinks/${editing}`, {
        ...newBread,
  ...
  const onDelete = (id) => {
    axios
      .delete(`http://localhost:5000/api/drinks/${id}`)
      .then(() => {
  ...
  const onAdd = () => {
    ...
    axios
      .post('http://localhost:5000/api/drinks', newBread) // Add new bread
      .then((response) => {
  ...
  return (
    <div className="management-pagee">
      <h2>Trang Quản lý Đồ Uống</h2>

      {/* Modal Form for Add/Edit Bread */}
      {modalVisible && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h3>{editing ? 'Sửa Đồ Uống' : 'Thêm Đồ Uống'}</h3>
            <form>
              <input
                type="text"
                placeholder="Tên đô uống"
                value={newBread.name}
