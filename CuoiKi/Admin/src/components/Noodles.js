...
const Noodless = () => {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/api/noodless')
      .then((response) => {
  ...
  const handleUpdate = () => {
    axios
      .put(`http://localhost:5000/api/noodless/${editing}`, {
        ...newBread,
  ...
  const onDelete = (id) => {
    axios
      .delete(`http://localhost:5000/api/noodless/${id}`)
      .then(() => {
  ...
  const onAdd = () => {
    ...
    axios
      .post('http://localhost:5000/api/noodless', newBread) // Add new bread
      .then((response) => {
  ...
  return (
    <div className="management-pagee">
      <h2>Trang Quản lý Mì </h2>

      {/* Modal Form for Add/Edit Bread */}
      {modalVisible && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h3>{editing ? 'Sửa  Mì' : 'Thêm Mì'}</h3>
            <form>
              <input
                type="text"
                placeholder="Tên Mì"
