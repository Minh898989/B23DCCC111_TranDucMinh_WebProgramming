...
const Foods = () => {
  ...
  const handleUpdate = () => {
    axios
      .put(`http://localhost:5000/api/foods/${editing}`, {
        ...newBread,
  ...
  const onAdd = () => {
    ...
    axios
      .post('http://localhost:5000/api/foods', newBread) // Add new bread
      .then((response) => {
  ...
  return (
    <div className="management-pagee">
      <h2>Trang Quản lý Gà </h2>

      {/* Modal Form for Add/Edit Bread */}
      {modalVisible && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h3>{editing ? 'Sửa Gà Ngon' : 'Thêm Gà Ngon'}</h3>
            <form>
              <input
                type="text"
                placeholder="Tên Gà"
