...
const Desserts = () => {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/api/dessert')
      .then((response) => {
  ...
  const handleUpdate = () => {
    axios
      .put(`http://localhost:5000/api/dessert/${editing}`, {
        ...newBread,
  ...
  const onDelete = (id) => {
    axios
      .delete(`http://localhost:5000/api/dessert/${id}`)
      .then(() => {
  ...
  const onAdd = () => {
    ...
    axios
      .post('http://localhost:5000/api/dessert', newBread) // Add new bread
      .then((response) => {
  ...
  return (
    <div className="management-pagee">
      <h2>Trang Quản lý Tráng Miệng</h2>

      {/* Modal Form for Add/Edit Bread */}
      {modalVisible && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h3>{editing ? 'Sửa Tráng Miệng' : 'Thêm Tráng Miệng'}</h3>
            <form>
              <input
                type="text"
                placeholder="Tên Tráng Miệng"
                value={newBread.name}
